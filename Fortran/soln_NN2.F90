...
#include "definition.h"
   ...
   real(kind=4), dimension(1, 3), target   :: output
   ...
   real(kind=8) :: meanDens, meanVel, meanPres, meanDiv, stdDens, stdVel, stdPres, stdDiv, softTotal
   ...
   do i = gr_ibeg - 1, gr_iend + 1
      ...
      softTotal = exp(output(1, 1)) + exp(output(1, 2)) + exp(output(1, 3))
      !if (exp(output(1, 2))/(exp(output(1, 1)) + exp(output(1, 2))) >= 0.99) then
      if (exp(output(1, 2))/softTotal >= 0.99) then
         classification = 1
      else if (exp(output(1, 3))/softTotal >= 0.99) then
         classification = 2
         !     !classification = 1.0
         !     !classification = 0.0
      else
   ...
   do i = gr_ibeg - 2, gr_iend + 2 ! 3 ghost cells !!!
      ...
      elseif (sim_charLimiting) then
         do kWaveNum = 1, NUMB_WAVE
            ...
            if (sim_norm_limiter == 'minmod') then
               call minmod(dot_product(leig(DENS_VAR:PRES_VAR, kWaveNum), delL(DENS_VAR:PRES_VAR)), &
                           ...
                           & delW(kWaveNum))
            elseif (sim_norm_limiter == 'vanLeer') then
               !elseif (predictions(i) == 2) then
               call vanLeer(dot_product(leig(DENS_VAR:PRES_VAR, kWaveNum), delL(DENS_VAR:PRES_VAR)), &
                           & dot_product(leig(DENS_VAR:PRES_VAR, kWaveNum), delR(DENS_VAR:PRES_VAR)), &
                           & delW(kWaveNum))
            elseif (sim_norm_limiter == 'mc') then
            ...
            elseif (sim_norm_limiter == "center") then
   ...
   do i = gr_ibeg - 1, gr_iend + 1
      ...
      if (predictions(i) == 1) then
         ...
         ! Not a shock (PPM)
      elseif ((predictions(i) == 0 .OR. predictions(i) == 2) .AND. (sim_shock_method == "ppm")) then
         if (predictions(i) == 2) then
            print *, "fdjafkl"
         end if
         call eigenvalues(gr_V(DENS_VAR:GAME_VAR, i), lambda)
         call left_eigenvectors(gr_V(DENS_VAR:GAME_VAR, i), conservative, leig)
        ...
        A_PLUS(nvar) = (1.0d0/2.0d0)*(gr_V(nVar, i) + gr_V(nVar, i + 1)) - (1.0d0/6.0d0)*(delVChar(nVar, i + 1) - delVChar(nVar, i))
         ...
         gr_vR(DENS_VAR:PRES_VAR, i) = C_MAT(DENS_VAR:PRES_VAR, 0) + sigR(DENS_VAR:PRES_VAR)
         ! Not a shock PLM
      elseif ((predictions(i) == 0 .OR. predictions(i) == 2) .AND. (sim_shock_method == "plm")) then
         conservative = .false.
         ...
         do kWaveNum = 1, NUMB_WAVE
            ...
            ! print *, sim_norm_limiter
            ! print *, sim_shock_limiter
            ! print *, sim_contact_limiter
            ! print *, sim_limiter
         end do
      else
         print *, "Hell"
